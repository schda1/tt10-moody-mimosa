...
Spi::Spi(SPI_TypeDef* instance) : instance(instance), cs(nullptr), mode(SpiMode::MODE_0), frequency(1000000) {}
...
{
    set_cs(0);
    HAL_SPI_Transmit(&hspi, data_tx, len, HAL_MAX_DELAY);
    set_cs(1);
}
...
{
    set_cs(0);
    HAL_SPI_Receive(&hspi, data_rx, len, HAL_MAX_DELAY);
    set_cs(1);
}
...
{
    set_cs(0);
    HAL_SPI_TransmitReceive(&hspi, data_tx, data_rx, len, HAL_MAX_DELAY);
    set_cs(1);
}
...
{
    set_cs(0);
    HAL_SPI_Transmit(&hspi, data_tx, len_tx, HAL_MAX_DELAY);
    HAL_SPI_Receive(&hspi, data_rx, len_rx, HAL_MAX_DELAY);
    set_cs(1);
}
...
{
    set_cs(0);
    HAL_SPI_Transmit(&hspi, data_tx1, len_tx1, HAL_MAX_DELAY);
    HAL_SPI_Transmit(&hspi, data_tx2, len_tx2, HAL_MAX_DELAY);
    set_cs(1);
}